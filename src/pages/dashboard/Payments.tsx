...
const Payments = () => {
  ...
  const toggleActive = async (id: string, currentState: boolean) => {
    try {
      await paymentService.togglePaymentLink(id, currentState);
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {paymentLinks.map((link) => (
            <div
              key={link.id}
              className={`bg-card border rounded-lg p-4 transition-colors ${link.is_active ? "border-border" : "border-border/50 opacity-60"}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full font-body text-[10px] font-medium ${link.is_active ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1 bg-muted border border-border rounded-lg px-2.5 py-2 font-body text-xs text-foreground hover:bg-primary/10 transition-colors"
                    title="Copy link"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 bg-muted border border-border rounded-lg px-2.5 py-2 font-body text-xs text-foreground hover:bg-green-500/10 transition-colors"
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 bg-muted border border-border rounded-lg px-2.5 py-2 font-body text-xs text-foreground hover:bg-accent transition-colors"
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 bg-muted border border-border rounded-lg px-2.5 py-2 font-body text-xs text-destructive hover:bg-destructive/10 transition-colors"
                    title="Delete"
                  >
