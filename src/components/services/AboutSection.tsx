      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 mb-16 max-w-5xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h3 className="font-heading text-xl font-bold text-foreground mb-4">Comprehensive Digital Solutions</h3>
          ...
          <div className="lg:col-span-2 flex flex-col gap-4 justify-center">
            {stats.map((stat, i) => (
              <motion.div
                key={stat.label}
                className="p-5 border border-primary/10 bg-background"
                initial={{ opacity: 0, x: 20 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
              ...
              >
                ...
                <div className="font-body text-sm text-foreground">{stat.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {highlights.map((h, i) => (
            <motion.div
              key={h.label}
              className="flex flex-col items-center gap-2 p-3 border border-primary/10 bg-background hover:border-primary/30 transition-colors"
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h3 className="font-heading text-xl font-bold text-foreground mb-4">Tech & Industry Agnostic Partner</h3>
          <p className="font-body font-light text-muted-foreground text-sm leading-relaxed">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {capabilities.map((c, i) => (
            <motion.div
              key={c.label}
              className="flex items-center gap-2 px-5 py-3 rounded-full border border-primary/10 bg-background hover:border-primary/30 transition-colors"
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.1 }}
            >
              ...
              <span className="font-subheading text-sm text-foreground font-medium">{c.label}</span>
            </motion.div>
          ))}
        </div>
      </div>
