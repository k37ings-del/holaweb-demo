...
import { ArrowLeft, Cloud, Code, TrendingUp, MessageSquare } from "lucide-react";
    ...
    return (
      <Layout>
        <div className="min-h-[60vh] flex items-center justify-center">
          <p className="text-muted-foreground font-poppins">Service not found.</p>
        </div>
      </Layout>
    );
  }
    ...
    <Layout>
      <section className="py-24 relative">
        <div className="container mx-auto px-4 lg:px-8">
          <Link
            to="/services"
            className="inline-flex items-center gap-2 text-golden text-sm font-noto mb-8 hover:underline"
          >
          ...
          <div className="flex items-start gap-6 mb-8">
            <div className="w-20 h-20 rounded-full border-2 border-golden/30 bg-golden/10 flex items-center justify-center flex-shrink-0">
              <IconComp className="w-9 h-9 text-golden" />
            </div>
            <div>
              <span className="inline-block px-3 py-1 rounded-full bg-golden/10 text-golden text-xs font-noto uppercase tracking-wider mb-2">
                {service.category}
              </span>
              <h1 className="font-garet text-3xl md:text-5xl font-bold text-foreground">
                {service.title}
              </h1>
            </div>
          </div>
          ...
          <div className="max-w-3xl mb-8">
            {service.intro.map((p, i) => (
              <p key={i} className="font-poppins text-muted-foreground text-lg leading-relaxed mb-4">
                {p}
              </p>
            ))}
          </div>
          ...
          {/* Image placeholder */}
          <div className="w-full max-w-3xl h-64 md:h-80 rounded-2xl border-2 border-golden/20 bg-muted/20 flex items-center justify-center mb-12">
            <div className="text-muted-foreground/50 text-sm font-poppins text-center">
              <div className="w-12 h-12 mx-auto mb-2 rounded-full bg-golden/10 flex items-center justify-center">
                <IconComp className="w-6 h-6 text-golden/50" />
              </div>
              Image Placeholder
            </div>
          </div>
          ...
          {/* Clickable features */}
          <h2 className="font-garet text-2xl font-bold text-foreground mb-6">What's Included</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mb-12">
            {service.features.map((f) => (
              <div key={f.name} className="rounded-xl border border-golden/10 bg-card overflow-hidden">
                <button
                  onClick={() => setOpenFeature(openFeature === f.name ? null : f.name)}
                  className="w-full flex items-center gap-3 p-4 text-left hover:bg-golden/5 transition-colors"
                >
                  <span className="w-2 h-2 rounded-full bg-golden flex-shrink-0" />
                  <span className="font-poppins text-sm text-foreground flex-1">{f.name}</span>
                  <span className={`text-golden transition-transform duration-200 ${openFeature === f.name ? "rotate-180" : ""}`}>
                    ▾
                  </span>
                </button>
                <AnimatePresence>
                  {openFeature === f.name && (
                    ...
                    >
                      <p className="px-4 pb-4 text-sm text-muted-foreground font-poppins leading-relaxed">
                        {f.summary}
                      </p>
                    </motion.div>
                  )}
          ...
          <Link to="/contact" className="btn-cherry inline-block text-base">
            Get Started →
          </Link>
        </div>
      </section>
